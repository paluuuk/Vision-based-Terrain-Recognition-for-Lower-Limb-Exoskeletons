...
if __name__ == "__main__":
    ...
    first = True
    alpha = 0.02
    camera_height = 0.63 # 0.68  # 0.92  # To be provided by Exoskeleton
    # *******************************************************************************
    ...
    while True:
        ...
        depth_colormap_original = cv2.normalize(depth_image, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        depth_colormap_original = cv2.applyColorMap(depth_colormap_original, cv2.COLORMAP_JET)
        
        # Depth map inpainting
        invalid_mask = (depth_image == 0).astype(np.uint8)
        depth_image_inpainted = cv2.inpaint(depth_image, invalid_mask, inpaintRadius=5, flags=cv2.INPAINT_NS)
        
        # Normalize depth image for display
        depth_colormap_inpainted = cv2.normalize(depth_image_inpainted, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        depth_colormap_inpainted = cv2.applyColorMap(depth_colormap_inpainted, cv2.COLORMAP_JET)
        
        # Convert depth image to CV_8U for denoising
        depth_image_uint8 = cv2.normalize(depth_image, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        
        # Depth map denoising
        denoised_depth_image = cv2.fastNlMeansDenoising(depth_image_uint8, h=10, templateWindowSize=7, searchWindowSize=21)
        
        # Normalize denoised depth image for display
        denoised_depth_colormap = cv2.applyColorMap(denoised_depth_image, cv2.COLORMAP_JET)

        # Temporal filtering
        depth_buffer.append(depth_image)
        if len(depth_buffer) > window_size:
            depth_buffer.pop(0)
        filtered_depth_image = np.mean(depth_buffer, axis=0).astype(np.uint16)

        # Normalize filtered depth image for display
        filtered_depth_colormap = cv2.normalize(filtered_depth_image, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        filtered_depth_colormap = cv2.applyColorMap(filtered_depth_colormap, cv2.COLORMAP_JET)
       
        cv2.imshow('Inpainted Depth Image', depth_colormap_inpainted)
        cv2.imshow('Depth Image original', depth_colormap_original)
        cv2.imshow('Denoised Depth Image', denoised_depth_colormap)
        cv2.imshow('Filtered Depth Image', filtered_depth_colormap)

        difference_map = np.abs(depth_image.astype('float32') - depth_image_inpainted.astype('float32'))
