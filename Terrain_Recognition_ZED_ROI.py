        ...
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            if text_rotation != 'No info':
                if classify_pts.shape[0] != 0:
                    ...
                    # finite_mask = np.all(np.isfinite(output_pts), axis=1)

       
                    # output_pts = output_pts[finite_mask, :]

                    # if output_pts.size > 0:
                    #     # Proceed with operations if output_pts is not empty
                    #     print("OP Limits 0: ", np.min(output_pts[:, 0]), np.max(output_pts[:, 0]))
                    #     # Ensure dimensions match for matrix multiplication
                    #     if output_pts.shape[1] == conv_mat.shape[1]:
                    #         points_to_classify = np.matmul(conv_mat, output_pts.T)  # Ensure dimensions are compatible
                    #         points_to_classify = points_to_classify.T  # Transpose back if needed
                    #         # Continue with any subsequent processing of points_to_classify
                    #     else:
                    #         # Handle the dimension mismatch scenario, possibly by adjusting conv_mat or output_pts
                    #         print("Dimension mismatch between conv_mat and output_pts.")
                    # else:
                    #     # Handle the case where output_pts is empty
                    #     print("output_pts is empty after removing non-finite values.")
                    #     # Skip operations that depend on output_pts
                    ...
                    if reduced_dim_points.shape[0] != 0:
                        ...
                        if points_to_consider.shape[0] != 0:
                            ...
                            cv2.imshow('Color Stream', background)
