...
if __name__ == "__main__":
    ...
    depth_buffer = []   
    MIN_DEPTH = 0.2  # Minimum depth value to visualize
    MAX_DEPTH = 5.0  # Maximum depth value to visualize

    def visualize_depth_data(depth_data, min_depth, max_depth, colormap=cv2.COLORMAP_JET):
        # Normalize depth data within the specified range for visualization
        depth_normalized = np.zeros_like(depth_data, dtype=np.float32)  # Initialize with zeros for invalid/out-of-range depths
        valid_mask = (depth_data >= min_depth) & (depth_data <= max_depth)
        depth_clipped = np.clip(depth_data, min_depth, max_depth)
        depth_normalized[valid_mask] = np.interp(depth_clipped[valid_mask], (min_depth, max_depth), (0, 255))
        # Convert to uint8 and apply colormap
        depth_colormap = cv2.applyColorMap(depth_normalized.astype(np.uint8), colormap)
    ...
    while True:
        ...
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            original_depth_colormap = visualize_depth_data(depth_array, MIN_DEPTH, MAX_DEPTH)
            cv2.imshow('Original Depth Image', original_depth_colormap)

            # Introduce a large, artificial gap for testing
            test_depth_array = np.copy(depth_array)
            test_depth_array[50:200, 100:300] = 0  # Creating a larger single gap
            # Creating multiple gaps
            test_depth_array[50:100, 50:100] = 0
            test_depth_array[150:200, 200:250] = 0
            test_depth_array[300:350, 100:150] = 0
            # Adjust the indices as needed based on your depth map size and areas of interest

            test_depth_array_colormap = visualize_depth_data(test_depth_array, MIN_DEPTH, MAX_DEPTH)
            cv2.imshow('test', test_depth_array_colormap)

            # Apply inpainting on this modified depth map
            invalid_mask_test = (test_depth_array == 0).astype(np.uint8)  # Creating a mask for the inpainting
            inpainted_test_array = cv2.inpaint(test_depth_array, invalid_mask_test, inpaintRadius=5, flags=cv2.INPAINT_NS)

            # Visualization of the inpainted result for the test array
            test_depth_colormap = visualize_depth_data(inpainted_test_array, MIN_DEPTH, MAX_DEPTH)
            cv2.imshow('Inpainted Test Depth Image', test_depth_colormap)
            
            # Inpainting
            invalid_mask = (depth_array == 0).astype(np.uint8)
            inpainted_depth_array = cv2.inpaint(depth_array, invalid_mask, inpaintRadius=10, flags=cv2.INPAINT_NS).astype(np.float32)
            inpainted_depth_colormap = visualize_depth_data(inpainted_depth_array, MIN_DEPTH, MAX_DEPTH)
            cv2.imshow('Inpainted Depth Image', inpainted_depth_colormap)
            
            # Denoising (ensure inpainted depth data is normalized and converted to uint8 for denoising)
            inpainted_normalized_uint8 = np.clip(inpainted_depth_array, MIN_DEPTH, MAX_DEPTH)
            inpainted_normalized_uint8 = np.interp(inpainted_normalized_uint8, (MIN_DEPTH, MAX_DEPTH), (0, 255)).astype(np.uint8)
            denoised_depth_array = cv2.fastNlMeansDenoising(inpainted_normalized_uint8, None, h=10, templateWindowSize=7, searchWindowSize=21)
            denoised_depth_colormap = cv2.applyColorMap(denoised_depth_array, cv2.COLORMAP_JET)
            cv2.imshow('Denoised Depth Image', denoised_depth_colormap)
            
            # Temporal Filtering
            depth_buffer.append(denoised_depth_array.astype(np.float32))
            if len(depth_buffer) > window_size:
                depth_buffer.pop(0)
            filtered_depth_array = np.mean(depth_buffer, axis=0).astype(np.uint8)
            filtered_depth_colormap = cv2.applyColorMap(filtered_depth_array, cv2.COLORMAP_JET)
            cv2.imshow('Filtered Depth Image', filtered_depth_colormap)

            difference_map = np.abs(test_depth_array.astype('float32') - inpainted_test_array.astype('float32'))
            difference_map_normalized = cv2.normalize(difference_map, None, 0, 255, cv2.NORM_MINMAX).astype('uint8')
            difference_colormap = cv2.applyColorMap(difference_map_normalized, cv2.COLORMAP_JET)
            cv2.imshow('Difference Map', difference_colormap)

            depth_array = inpainted_depth_array

            display_depth_array = display_depth_frame.get_data()
            center_y, center_x = depth_array.shape[0] // 2, depth_array.shape[1] // 2
            start_y = max(center_y - square_size // 2, 0)
            start_x = max(center_x - square_size // 2, 0)

            depth_array = depth_array[start_y:start_y + square_size, start_x:start_x + square_size]
            ...
            u_square = np.arange(start_x, start_x + square_size)
            v_square = np.arange(start_y, start_y + square_size)
            u_mat_square, v_mat_square = np.meshgrid(u_square, v_square)

            
            pcd_grid = np.zeros((square_size, square_size, 3))
            pcd_grid[:, :, 0] = (u_mat_square - cx) / fx * depth_array
            pcd_grid[:, :, 1] = (v_mat_square - cy) / fy * depth_array
            pcd_grid[:, :, 2] = depth_array

            display_square_region = display_depth_array[start_y:start_y + square_size, start_x:start_x + square_size]
            ...
            sd_pcd = sd_pcd[index]
            sd_pcd = sd_pcd[~np.isnan(sd_pcd).any(axis=1)] # trying to remove nan values
                ...
                if classify_pts.shape[0] != 0:
                    ...
                    if reduced_dim_points.shape[0] != 0:
                        ...
                        if points_to_consider.shape[0] != 0:
                            ...
                            color_image_square = color_image_ocv[start_y:start_y + square_size, start_x:start_x + square_size]
                            color_image_square_resized = cv2.resize(color_image_square, (500, 500)) 
                            frame_width = width  
                            frame_height = height
                            background = np.zeros((frame_height, frame_width, 3), dtype=np.uint8)
                            overlay_start_x = (frame_width - square_size) // 2
                            overlay_start_y = (frame_height - square_size) // 2

                            # Overlay the square region onto the black background
                            background[overlay_start_y:overlay_start_y + square_size, 
                                    overlay_start_x:overlay_start_x + square_size] = color_image_square
                            
                            # Add text over the overlay
                            cv2.putText(background, terrain_classes[detect_terrain], (50, 50), 
                            ...
                            if video:
                                ...
                                color_out = cv2.resize(background, (640, 480))
                                combined_frames = np.hstack((color_out, resize_binary))
                                out_vid.write(combined_frames)

            # cv2.imshow('Binary image', depth_array)
            # plt.imshow(display_depth_array, cmap="plasma")
            display_square_normalized = cv2.normalize(display_square_region, None, 0, 255, cv2.NORM_MINMAX)
            display_square_normalized = np.uint8(display_square_normalized)
            plt.imshow(display_square_normalized, cmap='gray')
            # plt.imshow(display_square_region, cmap="plasma")
